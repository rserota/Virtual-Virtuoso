...
public class triggerKickNotes : MonoBehaviour {

	public AudioSource kickAudioClip;
	...
	void Start () {
		...
		kickAudioClip = gameObject.GetComponent<AudioSource>();
	...
	void OnTriggerEnter(Collider other) {
		...
		if ( other.tag.Contains("hand") && mrsm.muted == false ) {
			...
			if ( mrsm.recording == true ) {
				noteArray[timeKeeper.tickInLoop].Add(new Note("kick", timeKeeper.tickInLoop, kickAudioClip.clip.name, kickAudioClip, 1f));
			}
