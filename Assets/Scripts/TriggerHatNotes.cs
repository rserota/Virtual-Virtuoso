...
public class TriggerHatNotes : MonoBehaviour {
		...
		if ( other.tag.Contains("hand") && mrsm.muted == false ) {
			...
			if ( hand.currentHandState == "Idle" ) {
				...
				if ( mrsm.recording == true ) {
					noteArray[timeKeeper.tickInLoop].Add(new Note("hatClosed", timeKeeper.tickInLoop, "hatClosed", notesDict["hatClosed"], 1f));
				}
			}

			else if ( hand.currentHandState == "Fist" ) {
				notesDict["hatOpen"].Play();
				if ( mrsm.recording == true ) {
					noteArray[timeKeeper.tickInLoop].Add(new Note("hatOpen", timeKeeper.tickInLoop, "hatOpen", notesDict["hatOpen"], 1f));
				}
			}
