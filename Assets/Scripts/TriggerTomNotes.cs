...
public class TriggerTomNotes : MonoBehaviour {
	...
	void OnTriggerEnter(Collider other) {
		...
		if ( other.tag.Contains("hand") && mrsm.muted == false ) {
			...
			if ( hand.currentHandState == "Idle" ) {
				notesDict["lowTom"].Play();
				if ( mrsm.recording == true ) {
					noteArray[timeKeeper.tickInLoop].Add(new Note("lowTom", timeKeeper.tickInLoop, "lowTom", notesDict["lowTom"], 1f));
				}
			}

			else if ( hand.currentHandState == "Fist" ) {
				notesDict["midTom"].Play();
				if ( mrsm.recording == true ) {
					noteArray[timeKeeper.tickInLoop].Add(new Note("midTom", timeKeeper.tickInLoop, "midTom", notesDict["midTom"], 1f));
				}
			}
